...
int main(int argc, char ** argv)
{
    if (argc<3)
    {
        fprintf(stderr, "usage: pagerank [path] [jobs] [iterations] [start vertex id] [cache size in MB] [graph size in MB] [memory budget in MB]\n");
        exit(-1);
    }
    std::string path = argv[1];//路径
    int PRO_NUM = atoi(argv[2]); //工作数量
    int iterations = atoi(argv[3]);  //迭代次数
    VertexId start_vid = atoi(argv[4]);  //开始点
    long cache_size = atol(argv[5])*1024l*1024l;     //缓存大小
    long graph_size = atol(argv[6])*1024l*1024l;     //图大小
    long memory_bytes = (argc>=8)?atol(argv[7])*1024l*1024l:64*1024l*1024l;    //内存大小

    int parallelism = std::thread::hardware_concurrency();      //可同时执行线程数
    ...
    Graph graph(path);                    //载入图
    graph.set_memory_bytes(memory_bytes);  //设置图大小
	 //=malloc(memory_bytes);
	 Chunk_memory chunk_memory;
	chunk_memory.init(4980736,parallelism,memory_bytes,path);

    BigVector<float> pagerank[PRO_NUM];    //创建工作数量的pagerank容器
    ...
    //sssp
    Bitmap * active_in_sssp[PRO_NUM];    //跳过一些不活跃的顶点，一个bitmap大小应该等于块的顶点数。0不活跃1活跃
    ...
    #pragma omp barrier       //初始化部分

    graph.set_sizeof_blocks(cache_size, graph_size, vertex_data_bytes*4*PRO_NUM);

    graph.get_should_access_shard(graph.should_access_shard, nullptr);     //全ture
    graph.stream_edges<VertexId>(       //加载活动分区
        [&](Edge & e){
