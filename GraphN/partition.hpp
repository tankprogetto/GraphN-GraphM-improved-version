...
inline size_t get_partition_id(const size_t vertices, const size_t partitions, const size_t vertex_id) {      //当前节点所在块
...
inline std::pair<size_t, size_t> get_partition_range(const size_t vertices, const size_t partitions, const size_t partition_id) {      //当前进程所在块起终点
...
inline std::pair<size_t, size_t> get_chunk_range(const size_t vertices, const size_t partitions, const size_t partition_id) {      //当前进程所在chunk块起终点
        const size_t split_partition = vertices % (partitions*partitions);
        const size_t partition_size = vertices / (partitions*partitions) + 1;
        if (partition_id < split_partition) {
                const size_t begin = partition_id * partition_size;
                const size_t end = (partition_id + 1) * partition_size;
                return std::make_pair(begin, end);
